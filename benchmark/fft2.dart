...
/** Computes FFT's of complex, double precision data where n is an integer power of 2.
 ...
 class FFT {
  ...
  /** Compute Fast Fourier Transform of (complex) data, in place.*/
   static void transform (Float64List data) {
    transform_internal(data, -1); }

  /** Compute Inverse Fast Fourier Transform of (complex) data, in place.*/
   static void inverse (Float64List data) {
    transform_internal(data, +1);  
    // Normalize
    int nd=data.length;
    int n =(nd/2).toInt();
  ...
  /** Accuracy check on FFT of data. Make a copy of data, Compute the FFT, then
   ...
   static double test(Float64List data){
    int nd = data.length;
    // Make duplicate for comparison
    //Float64List copy = new Float64List(nd);
    //System.arraycopy(data,0,copy,0,nd);
    //Float64List copy = new Float64List(nd);
    //Arrays.copy(data,0,copy,0,nd);
    
    Float64List copy = new List.from(data);
  ...
  /** Make a random array of n (complex) elements. */
   static Float64List makeRandom(int n){
    int nd = 2*n;
    Float64List data = new Float64List(nd);
    for(int i=0; i<nd; i++) {
      data[i]= random();
    }
    return data; }
  ...
  /* ______________________________________________________________________ */
   ...
   static void transform_internal (Float64List data, int direction) {
   ...
   static void bitreverse(Float64List data) {
 ...
 double measureFFT(int N, double mintime) {
   ...
   Float64List x = RandomVector(2*N);
   Float64List oldx = NewVectorCopy(x);
   int cycles = 1;
   var elapsedTime;
