...
.. Stubが存在する理由は、JITコンパイルしたコードから直接Runtimeを呼び出すのではなく、以下の処理を生成コード側で行うための関節的なレイヤーになります。
.. 1. Dart VMのRuntime処理ごとに異なる、CallingConvの差異の吸収。
.. 2. 飛び先を動的に切り替える、たとえば未コンパイルソースコードのJITコンパイル処理を呼ぶか、JITコンパイル済みコードに飛ぶか。
...
InvokeやOptimizeFunctionがありますが、Dart srcとJITコンパイル済みCodeの管理がIsolateごとに分かれているためです。

InlineCache用のStubが多いですが、これもJITコンパイルしたCodeに依存するためです。

AllocateContextやUpdateStoreBufferは、HeapやGC向けの処理になります。これらもIsolateごとに分かれています。

まとめ
===============================================================================
Dart VMの構成要素として、ざっくり以下に分けられると考えています。
...
Isolate -> Class or Function -> ContextScopeなどから定義されています。

他にも、Closure、LocalScopeなどがあり、function_level, loop_level, context_levelなどに応じて、
ネストして定義されます。

それぞれのScopeから、以下のObjectPoolにひもづけられます。
...
Objectクラス系が、主に操作を担当して、RawObjectが実データを定義します。

そのため、GCの対象はすべてRawObjectになります。

Objectクラスのメソッドは、BootstrapNativesによって外部に公開されます。

JITコンパイラ
===============================================================================
Dart VMのJITコンパイラは、以下のパーツに分解されます。

1. parser(DartのソースコードをparseしてASTに変換する処理)
2. AST(各種ASTNodeの定義)
...
リソース管理
===============================================================================
Dart VMのリソース管理は、ざっくりいって以下の3種類です。
